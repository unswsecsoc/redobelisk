...
class CTFLevel(models.Model):
    ...
    def __str__(self):
        return "CTFLevel %s from %s, worth %d" % (self.name, self.ctf.name,
                                                  self.points)
...
class CTFAccount(models.Model):
    ctf = models.ForeignKey(CTF)
    user = models.ForeignKey(UserProfile)
    score = models.IntegerField()
    username = models.CharField(max_length=50)

    def __str__(self):
        return "CTFAccount %s from CTF %s user %s points %s" % (
            self.username, self.ctf.name, self.user.user.name, self.score)
